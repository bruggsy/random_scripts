...
with open(WORK_DIR + 'downloads/metaSRA/metasra.v1.1.json','r') as in_file:
...
with open(WORK_DIR + 'data/recount/metadata/ontoterm_to_subjects_gtex.json','w') as out_file:
    json.dump(mapped_metadata,out_file)

with open(WORK_DIR + 'data/recount/metadata/gtex_not_in_metasra.json','w') as out_file:
...
with open(WORK_DIR + 'data/recount/metasra/term_tree_gtex.json','w') as out_file:
...
for key in term_2_subjtree_metasra.keys():
    ...
    del term_entry['name']
    row.append(json.dumps(term_entry))

    out_array.append(row)

with open(WORK_DIR + 'data/recount/metasra/term_tree_table_gtex.csv','w') as out_file:
