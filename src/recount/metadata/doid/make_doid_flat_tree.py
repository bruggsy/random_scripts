...
# for id in doid_children.keys():

#     children_entry = doid_children[id]
...
#     doid_keys[id] = part_ids


terms_to_process = ['DOID:4']
...
while len(terms_to_process) > 0:
    ...
    term_children = doid_children.get(term,{}).get('children',[])
    # term_children = doid_children[term]['children']

    for child in term_children:
        
        child_key = all_keys[term]+'.'+child.split(':')[1]
...
for term_id in all_keys.keys():
    ...
    ## further filtering based off entries in doid_children here

    term_name = doid_children.get(term_id,{}).get('name',None)
...
with open(WORK_DIR + 'data/recount/metadata/doid/doid_tree_flat.json','w') as out_file:
...
#     curr_level = copy.deepcopy(nested_tree['DOID:4: disease'])
...
#     nested_tree['DOID:4: disease'] = new_level
